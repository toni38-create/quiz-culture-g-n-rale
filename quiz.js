...
function choisirQuiz(quizId) {
    ...
    let currentQuiz = quizId; // Mettre à jour le quiz courant
    let currentQuestionIndex = 0; // Réinitialiser l'index de la question
    let points = 0; // Réinitialiser les points au début du quiz
    ...
    if (answerIndex === correctAnswer) {
        ...
        setTimeout(() => {
            ...
            if (questionIndex + 1 < questions[quizId].length) {
                questionIndex++;
                afficherQuestion(quizId, questionIndex);
                document.getElementById(quizId).style.display = 'block'; // Réaffiche le quiz pour la question suivante
            } else {
                // Si le quiz est terminé
                document.getElementById('imageContainer').style.display = 'flex'; // Affiche l'écran d'accueil
            }
